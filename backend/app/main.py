...
class AugmentationConfig(BaseModel):
    ...
    noise_intensity: float = Field(default=0.5, ge=0.0, le=1.0)
    blur: bool = False
...
def _build_pipeline(config: AugmentationConfig) -> A.Compose:
    ...
    if config.gaussian_noise:
        var_min = 10.0 + (config.noise_intensity * 70.0)
        var_max = 35.0 + (config.noise_intensity * 165.0)
        transforms.append(A.GaussNoise(var_limit=(var_min, var_max), mean=0, p=0.75))
    if config.blur:
